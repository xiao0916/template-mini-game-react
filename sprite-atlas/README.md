...
运行时通过 React `style` 写入的 CSS 变量不会经过构建器的 PostCSS 单位转换。因此 `createSpriteAtlasStyle` 会自行将设计稿 px 转为 CSS 单位。单位、设计稿宽度和 rem 根值统一维护在 `src/game/css-config.ts`，并由 `game-sdk.config.ts` 的 `css` 配置复用；切换 `unit` 后，构建期与运行时会保持一致。

`src/utils/css-unit.ts` 同时提供 `px2vw`、`px2rem`、`vw2px` 与 `rem2px`，其默认设计基准也来自该共享配置。
